...
from Multi_Radar.data_augmentation import X_train, Y_train_onehot
import absl.logging
...
output = Dense(4, activation='softmax')(f)
...
tensorboard = TensorBoard(log_dir='./logs/logging1-2')
early_stopper = EarlyStopping(monitor='val_loss', min_delta=0.001, patience=14, verbose=1, mode='auto')
checkpoint = ModelCheckpoint('weights/weight1-2/model_best_noenhance.h5', save_best_only=True, monitor='val_loss', mode='min')
X_train_split, X_val_fold, Y_train_split, y_val_fold_onehot = train_test_split(
    X_train, Y_train_onehot, test_size=0.2, random_state=random_seed)
